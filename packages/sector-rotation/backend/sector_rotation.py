...
這份檔案是從 investreport 專案的 backend/routers/stocks.py 原樣複製出來的
子集，只保留輪動圖相關的部分，方便獨立閱讀／分享。**不是**可直接執行的模組
——它假設被掛載在一個既有的 FastAPI router 上，並依賴主專案裡的其他模組
（見下方「外部依賴」）。實際程式碼以 backend/routers/stocks.py 為準，這份
檔案不會被自動同步更新。

詳見同目錄 README.md 的架構與演算法說明。

外部依賴（來自主專案，這裡未附上）：
    - `router`            : FastAPI APIRouter 實例（prefix="/stocks"）
    - `ns`                : backend/nstock.py，提供 ns.get_daily(stock_id) 讀取個股日K
    - `User`, `get_current_user` : backend/models.py, backend/routers/auth.py（登入驗證）
    - `genai`, `genai_types`, `_SAFETY_OFF` : google-genai SDK 初始化（見 stocks.py 開頭）
    - 環境變數 `GEMINI_API_KEY`
"""
